...
covariates, which, left untreated, inflates out-of-sample AUC to 0.9999. All code and artifacts are
public.
\end{abstract}
...
This paper takes up both questions on open data. We construct a monthly discrete-time hazard panel from the Fannie
...
evaluate them at two levels: loan-level discrimination and, more importantly for economic
purposes, UPB-weighted cohort-level CPR error against full-population actuals.
...
metrics have limited economic meaning for portfolio speeds; as Section~\ref{sec:leakage}
illustrates, they are also the metrics most easily corrupted by leakage.
...
waivers, media effects) are not spanned by loan-level covariates, and we treat this as a
limitation rather than attempting to tune it away. The AUC column also deserves comment: discrimination
...
2022--25 (Figure~\ref{fig:e1}). The onset is severe: in January 2022 the frozen model predicts
...
repriced. Over 2022 as a whole its UPB-weighted cohort error is 3.7 CPR points. The annual
errors show that the frozen model does not improve thereafter: 3.8 points in 2023, 3.3 in 2024,
3.1 in 2025. The errors do not decay as the lock-in
...
essentially every month after mid-2022. Re-estimation recovers, but only as new-regime
observations accumulate: folding in the 2022 transition year nearly halves the 2023 error (2.2
versus 3.8 points), and with two full years of post-break observations the annually
re-estimated model reaches 0.9 points in both 2024 and 2025, comparable to pre-break accuracy.
The governance implication is twofold: a structurally broken model cannot be ridden out in the
hope that its errors mean-revert, and recalibration, while effective, is gradual. Both
implications are consistent with the documented difficulties of commercial models over 2023--25
\citep{yu2023uncharted, katz2024models}.
...
2019-12; refinancing wave, 2020-04 to 2021-12; lock-in, 2022-07 to 2025-12) and trace
...
Interpreting Table~\ref{tab:e2} requires care. Partial-dependence values at a
...
triple over the same interval and constitute the most rate-sensitive segment of the stack. This is the model-implied
...
overlays in the interim. They also argue for close attention to the steep embedded optionality
of the 2023--24 high-coupon stack. For researchers, the removal-record leakage taxonomy of
