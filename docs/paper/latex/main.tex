...
function changed after the 2022 interest-rate shock. We report three findings. First, the
regime break was severe and, for a frozen model, \emph{persistent}: a model estimated through
2021 over-predicts aggregate speeds by approximately 9 CPR points at the onset of the shock and
remains 3.1--3.8 CPR points wrong in every subsequent year through 2025---the errors do not
mean-revert. Annual re-estimation nearly halves cohort error in its first post-break year (2.2
versus 3.8 CPR points in 2023) but requires roughly two years of new-regime data before errors
fall below one CPR point. Second, partial-dependence analysis of the
...
Our contributions are threefold. First, we quantify both the persistence of legacy-model error
and the recovery path of re-estimation across the break (Section~\ref{sec:e1}): the pre-shock
model fails immediately and its errors never mean-revert---3.1 to 3.8 CPR points of UPB-weighted
cohort error in every year from 2022 through 2025---while annual re-estimation improves steadily
yet requires roughly two years of new-regime data to bring cohort errors below one CPR point.
The implication for model governance is that a structurally broken model cannot be ``ridden
out,'' and that recalibration restores accuracy only at the pace at which new-regime data
accumulates; both facts are consistent with the documented performance of commercial models over
2023--25. Second, using per-regime estimation and partial-dependence analysis on a fixed
...
gradient-boosted model dominates the logistic benchmark on cohort-level error in six of seven
windows, decisively so in the lock-in era (0.9 versus 6.1 CPR points at the 2024 split). Second,
...
discrimination (AUC 0.75/0.65/0.64), ahead of the trees on cohort error at the pre-wave split
(8.8 versus 12.2 CPR points) but behind in the lock-in era (4.6 versus 3.7 and 1.5 versus 0.9)
and under-predictive of the hazard level; at $2.5$ million training rows this mixed result is
consistent with the tabular-learning literature, and the original deep-learning advantage of
\citet{sadhwani2021deep} was demonstrated on three orders of magnitude more data.
...
2022--25 (Figure~\ref{fig:e1}). Two facts emerge from the error record. The first is the
severity of the onset: in January 2022 the frozen model predicts aggregate speeds of
approximately 19 CPR against an actual print of 11, an error of nearly 9 CPR points, as it
extrapolates refinancing-wave behavior into a repriced market; over 2022 as a whole its
UPB-weighted cohort error is 3.7 CPR points. The second is the \emph{persistence} of the frozen
model's error: 3.8 points in 2023, 3.3 in 2024, and 3.1 in 2025. The errors do not decay as the
lock-in era matures, because the model's learned response surface---not merely its level---is
wrong for the new regime; Figure~\ref{fig:e1} shows it running 2--4 points above realized speeds
in essentially every month after mid-2022. Re-estimation, by contrast, recovers steadily:
including the 2022 transition year nearly halves the 2023 error (2.2 versus 3.8 points), and
with two full years of post-break data the annually re-estimated model reaches 0.9 points in
both 2024 and 2025, comparable to pre-break accuracy. The governance implication is symmetric: a
structurally broken model cannot be ridden out in the hope that its errors mean-revert, and
recalibration works but restores accuracy only at the pace at which new-regime observations
accumulate. Both facts are consistent with the documented difficulties of commercial models over
