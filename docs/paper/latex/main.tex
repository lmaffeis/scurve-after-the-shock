...
This paper provides, to our knowledge, the first public model-side treatment of the 2022 regime
break estimated on open data. We construct a monthly discrete-time hazard panel from the Fannie
...
information available at the beginning of month $t$. Cohort-level speeds aggregate fitted hazards
with combined sampling and balance weights: for cohort $c$ in month $t$,
\begin{equation}
...
\end{equation}
where $w_{it}$ is the inverse-probability sampling weight and $b_{it}$ the balance entering the
month.
...
structural, are stable and partially offsetting. Re-estimation overtakes the frozen benchmark
only in 2024 (2.6 versus 2.1) and wins decisively in 2025 (1.1 versus 2.4). The governance
implication: after a structural break, immediate recalibration on the transition sample can be
value-destroying; approximately two years of new-regime data were required before re-estimation
dominated. This is consistent with the documented 2023--25 whipsaw in commercial model
...
average slope between 0 and $+150$ basis points of incentive.}
\label{tab:e2}
\end{table}
...
through a degraded incentive response. SATO-conditioned partial dependence (replication
...
Using open data and commodity hardware, we provide the first public model-side account of the
